...
export class MatrixSpacingCalculator {
    ...
    calculateSpacing(
        ...
        const matrixASpacing = this.calculateMatrixSpacing(
            aRows,
            aCols,
            widthPerCol * aCols,
            availableHeight,
        );
        const matrixBSpacing = this.calculateMatrixSpacing(
            bRows,
            bCols,
            widthPerCol * bCols,
            availableHeight,
        );
        const matrixCSpacing = this.calculateMatrixSpacing(
            cRows,
            cCols,
            widthPerCol * cCols,
            availableHeight,
        ...
        if (constraints.maintainAspectRatio) {
            const minCellSize = Math.min(
                matrixASpacing.cellWidth,
                matrixASpacing.cellHeight,
                matrixBSpacing.cellWidth,
                matrixBSpacing.cellHeight,
                matrixCSpacing.cellWidth,
                matrixCSpacing.cellHeight,
            );
        ...
        return {
            ...
            symbols: {
                multiply: {
                    ...
                    height: Math.max(
                        matrixASpacing.height,
                        matrixBSpacing.height,
                    ),
                },
                equals: {
                    width: this.config.symbolWidth,
                    height: matrixCSpacing.height,
                },
            },
            totalWidth:
                matrixASpacing.width +
                matrixBSpacing.width +
                matrixCSpacing.width +
                this.config.symbolWidth * 2 +
                this.config.margins * 2,
            totalHeight:
                Math.max(
     ...
     */
    private calculateMatrixSpacing(
        rows: number,
        cols: number,
        maxWidth: number,
        maxHeight: number,
    ...
    calculateMinimumSpace(
        ...
        const minWidth =
            totalCols * this.config.minCellSize +
            this.config.symbolWidth * 2 +
            this.config.margins * 2;
        const minHeight =
            maxRows * this.config.minCellSize + this.config.margins * 2;
...
export function createSpacingCalculator(
    config?: Partial<SpacingConfig>,
): MatrixSpacingCalculator {
    const finalConfig = config
        ? { ...DEFAULT_SPACING_CONFIG, ...config }
        : DEFAULT_SPACING_CONFIG;
