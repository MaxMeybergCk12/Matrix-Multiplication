...
function calculateElementPositions(
  ...
  for (let row = 0; row < rows; row++) {
    for (let col = 0; col < cols; col++) {
      const x = padding + BRACKET_WIDTH + horizontalGap + (col * (elementSize + horizontalGap));
...
function calculateMatrixStructure(
  ...
  const { elementSize, horizontalGap, verticalGap, padding } = calculateElementSpacing(
    rows, cols, containerWidth, containerHeight
  );
  
  // Calculate element positions
  const positions = calculateElementPositions(
    rows, cols, elementSize, horizontalGap, verticalGap, padding
  );
  
  // Calculate bracket positioning
  const bracketDimensions = calculateBrackets(containerWidth, containerHeight);
